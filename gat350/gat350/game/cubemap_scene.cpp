...
bool CubemapScene::Create(const Name& name)
{
	...
	//{
	//	auto texture = std::make_unique<Texture>();
	//	std::vector<std::string> suffixes = { "lf", "rt", "up", "dn", "ft", "bk" };
	//	std::vector<std::string> names = Texture::GenerateCubeMapNames("textures/bluefreeze_", suffixes, ".tga");
	//	texture->CreateCubeTexture(names);
	//	m_engine->Resources()->Add("bluefreeze_texture", std::move(texture));
	//}

	//{
	//	auto texture = std::make_unique<Texture>();
	//	std::vector<std::string> suffixes = { "rt", "lf", "up", "dn", "bk", "ft" };
	//	std::vector<std::string> names = Texture::GenerateCubeMapNames("textures/darkskies_", suffixes, ".tga");
	//	texture->CreateCubeTexture(names);
	//	m_engine->Resources()->Add("darkskies_texture", std::move(texture));
	//}

	{
		auto texture = std::make_unique<Texture>();
		std::vector<std::string> suffixes = { "east", "west", "up", "down", "north", "south" };
		std::vector<std::string> names = Texture::GenerateCubeMapNames("textures/clouds1_", suffixes, ".bmp");
		texture->CreateCubeTexture(names);
		m_engine->Resources()->Add("clouds_texture", std::move(texture));
	}

	//auto texture = m_engine->Resources()->Get<Texture>("textures/grid.png");
	material->textures.push_back(m_engine->Resources()->Get<Texture>("clouds_texture"));
...
void CubemapScene::Update()
{
	...
	auto shader = m_engine->Resources()->Get<Program>("reflection_shader").get();
	shader->Use();
	...
	ImGui::SliderFloat("index", &(m_index), 0.5f, 3.0f);
	//model->Edit();
	//light->Edit();
