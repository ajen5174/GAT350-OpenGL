...
bool MultiLightScene::Create(const Name& name)
{
	...
	shader->Link();
	m_engine->Resources()->Add("phong_shader_lights", std::move(shader));
	...
	model->m_shader = m_engine->Resources()->Get<Program>("phong_shader_lights");
	Add(std::move(model));
...
void MultiLightScene::Update()
{
	...
	auto shader = m_engine->Resources()->Get<Program>("phong_shader_lights");
	// set shader uniforms
